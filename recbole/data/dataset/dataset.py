...
from recbole.data.utils import split_user_group
...
class Dataset(object):
    ...
    def __init__(self, config):
        ...
        self._from_scratch()
        self.g1_user_list = None  # 分组名单！！！
        self.g2_user_list = None
    ...
    def _from_scratch(self):
        ...
        """
        self.logger.debug(
            set_color(f'Loading {self.__class__} from scratch.', 'green'))
    ...
    def _get_field_from_config(self):
        ...
        self.logger.debug(
            set_color('uid_field', 'blue') + f': {self.uid_field}')
        self.logger.debug(
            set_color('iid_field', 'blue') + f': {self.iid_field}')
    ...
    def _get_download_url(self, url_file, allow_none=False):
        ...
        with open(
                os.path.join(
                    current_path,
                    f'../../properties/dataset/{url_file}.yaml')) as f:
                ...
                if os.path.isfile(file_path):
                    ...
                    for field in self.field2seqlen:
                        overall_field2seqlen[field] = max(
                            overall_field2seqlen[field],
                            self.field2seqlen[field])
                else:
    ...
    def _load_additional_feat(self, token, dataset_path):
        ...
        for suf in self.config['additional_feat_suffix']:
            ...
            if os.path.isfile(feat_path):
                ...
                raise ValueError(
                    f'Additional feature file [{feat_path}] not found.')
    ...
    def _get_load_and_unload_col(self, source):
        ...
        if self.config['unload_col'] is not None and source in self.config[
                'unload_col']:
        ...
        self.logger.debug(
            set_color('\t unload_col', 'blue') + f': [{unload_col}]')
    ...
    def _load_feat(self, filepath, source):
        ...
        self.logger.debug(
            set_color(
                f'Loading feature from [{filepath}] (source: [{source}]).',
                'green'))
        ...
        for field_type in head.split(field_separator):
            ...
            try:
                ...
                raise ValueError(
                    f'Type {ftype} from field {field} is not supported.')
            ...
            dtype[
                field_type] = np.float64 if ftype == FeatureType.FLOAT else str
        ...
        df = pd.read_csv(filepath,
                         delimiter=self.config['field_separator'],
                         usecols=usecols,
                         dtype=dtype,
                         encoding=encoding,
                         engine='python')
        ...
        for field in columns:
            ...
            if ftype == FeatureType.TOKEN_SEQ:
                ...
                df[field] = [
                    np.array(
                        list(map(float, filter(None, _.split(seq_separator)))))
                    for _ in df[field].values
                ]
    ...
    def _init_alias(self):
        ...
        for alias_name_1, alias_1 in self.alias.items():
            for alias_name_2, alias_2 in self.alias.items():
                if alias_name_1 != alias_name_2:
                    intersect = np.intersect1d(alias_1,
                                               alias_2,
                                               assume_unique=True)
                    if len(intersect) > 0:
                        raise ValueError(
        ...
        for alias_name, alias in self.alias.items():
            ...
            self._rest_fields = np.setdiff1d(self._rest_fields,
                                             alias,
                                             assume_unique=True)
    ...
    def _user_item_feat_preparation(self):
        ...
        if self.user_feat is not None:
            ...
            self.user_feat = pd.merge(new_user_df,
                                      ...
                                      how='left')
            self.logger.debug(
                set_color('ordering user features by user id.', 'green'))
        if self.item_feat is not None:
            ...
            self.item_feat = pd.merge(new_item_df,
                                      ...
                                      how='left')
            self.logger.debug(
                set_color('ordering item features by item id.', 'green'))
    ...
    def _preload_weight_matrix(self):
        ...
        for preload_id_field, preload_value_field in preload_fields.items():
            if preload_id_field not in self.field2source:
                raise ValueError(
                    f'Preload id field [{preload_id_field}] not exist.')
        ...
        for feat_name in self.feat_name_list:
            ...
            for field in feat:
                ...
                if ftype == FeatureType.TOKEN:
                    ...
                    feat[field] = feat[field].apply(lambda x: np.array(
    ...
    def _normalize(self):
        ...
        self.logger.debug(
            set_color('Normalized fields', 'blue') + f': {fields}')
    ...
    def _remove_duplication(self):
        ...
        if self.time_field in self.inter_feat:
            self.inter_feat.sort_values(by=[self.time_field],
                                        ascending=True,
                                        inplace=True)
            self.logger.info(
    ...
    def _filter_by_inter_num(self):
        ...
        user_inter_num = Counter(self.inter_feat[
            self.uid_field].values) if user_inter_num_interval else Counter()
        item_inter_num = Counter(self.inter_feat[
            self.iid_field].values) if item_inter_num_interval else Counter()
    ...
    def _get_illegal_ids_by_inter_num(self,
                                      field,
                                      feat,
                                      inter_num,
                                      inter_interval=None):
        ...
        if feat is not None:
            ...
            for id_ in feat[field].values:
                if inter_num[id_] < min_num:
                    ids.add(id_)
        self.logger.debug(
            f'[{len(ids)}] illegal_ids_by_inter_num, field=[{field}]')
        return ids
    ...
    def _parse_intervals_str(self, intervals_str):
        ...
        for endpoint_pair_str in str(intervals_str).split(';'):
            ...
            left_bracket, right_bracket = endpoint_pair_str[
                0], endpoint_pair_str[-1]
            endpoint_pair = endpoint_pair_str[1:-1].split(',')
            if not (len(endpoint_pair) == 2 and left_bracket in ['(', '[']
                    and right_bracket in [')', ']']):
                self.logger.warning(
                    f'{endpoint_pair_str} is an illegal interval!')
                continue

            left_point, right_point = float(endpoint_pair[0]), float(
                endpoint_pair[1])
            ...
            endpoints.append(
                (left_bracket, left_point, right_point, right_bracket))
        return endpoints
    ...
    def _within_intervals(self, num, intervals):
        ...
        """
        result = True
        for i, (left_bracket, left_point, right_point,
                right_bracket) in enumerate(intervals):
    ...
    def _filter_by_field_value(self):
        ...
        val_intervals = [] if self.config[
            'val_interval'] is None else self.config['val_interval']
        self.logger.debug(
            set_color('drop_by_value', 'blue') + f': val={val_intervals}')
        ...
        for field in val_intervals:
            ...
            if self.field2type[field] not in {
                ...
                raise ValueError(
                    f'Field [{field}] is not float-like field in dataset, which can\'t be filter.'
                )

            field_val_interval = self._parse_intervals_str(
                val_intervals[field])
            for feat in self.field2feats(field):
                feat.drop(feat.index[~self._within_intervals(
    ...
    def _del_col(self, feat, field):
        ...
        for dct in [
                self.field2id_token, self.field2token_id, self.field2seqlen,
                self.field2source, self.field2type
        ]:
            if field in dct:
                del dct[field]
    ...
    def seq_fields(self):
        ...
        """
        return self.fields(
            ftype=[FeatureType.FLOAT_SEQ, FeatureType.TOKEN_SEQ])
    ...
    def set_field_property(self, field, field_type, field_source,
                           field_seqlen):
    ...
    def counter(self, field):
        ...
        if field not in self.inter_feat:
            raise ValueError(
                f'Field [{field}] is not defined in ``inter_feat``.')
    ...
    def avg_actions_of_users(self):
        ...
        if isinstance(self.inter_feat, pd.DataFrame):
            ...
            return np.mean(
                list(
                    Counter(self.inter_feat[self.uid_field].numpy()).values()))
    ...
    def avg_actions_of_items(self):
        ...
        if isinstance(self.inter_feat, pd.DataFrame):
            ...
            return np.mean(
                list(
                    Counter(self.inter_feat[self.iid_field].numpy()).values()))
    ...
    def __str__(self):
        ...
        if self.uid_field:
            info.extend([
                set_color('The number of users', 'blue') +
                f': {self.user_num}',
        ...
        if self.iid_field:
            info.extend([
                set_color('The number of items', 'blue') +
                f': {self.item_num}',
                set_color('Average actions of items', 'blue') +
                f': {self.avg_actions_of_items}'
            ])
        info.append(
            set_color('The number of inters', 'blue') + f': {self.inter_num}')
    ...
    def split_by_ratio(self, ratios, group_by=None):
        ...
        #print('ratio_split next_df', next_df[0]['user_id'])
        self.g1_user_list, self.g2_user_list = split_user_group(
            next_df[0]['user_id'], self.dataset_name)  # 获得用户活跃度分组
    ...
    def leave_one_out(self, group_by, leave_one_mode):
        ...
        self._drop_unused_col()
        # [train_Interaction, valid_Interaction, test_Interaction]
        next_df = [self.inter_feat[index] for index in next_index]

        # print('leave_one_out next_df', next_df[0]['user_id'])
        self.g1_user_list, self.g2_user_list = split_user_group(
            next_df[0]['user_id'], self.dataset_name)  # 获得用户活跃度分组
    ...
    def build(self):
        ...
        if not isinstance(split_args, dict):
            raise ValueError(
                f'The split_args [{split_args}] should be a dict.')
        ...
        if split_mode == 'RS':
            if not isinstance(split_args['RS'], list):
                raise ValueError(
                    f'The value of "RS" [{split_args}] should be a list.')
            if group_by is None or group_by.lower() == 'none':
                datasets = self.split_by_ratio(split_args['RS'], group_by=None)
            elif group_by == 'user':
                datasets = self.split_by_ratio(split_args['RS'],
            ...
            datasets = self.leave_one_out(group_by=self.uid_field,
                                          leave_one_mode=split_args['LS'])
        else:
            raise NotImplementedError(
    ...
    def save(self):
        ...
        file = os.path.join(
            ...
            f'{self.config["dataset"]}-{self.__class__.__name__}.pth')
        self.logger.info(
    ...
    def _history_matrix(self, row, value_field=None):
        ...
        user_ids, item_ids = self.inter_feat[
            self.uid_field].numpy(), self.inter_feat[self.iid_field].numpy()
        if value_field is None:
    ...
    def _dataframe_to_interaction(self, data: object) -> object:
        ...
        for k in data:
            ...
            if ftype == FeatureType.TOKEN:
                ...
                seq_data = [
                    torch.LongTensor(d[:self.field2seqlen[k]]) for d in value
                ]
                new_data[k] = rnn_utils.pad_sequence(seq_data,
                                                     batch_first=True)
            elif ftype == FeatureType.FLOAT_SEQ:
                seq_data = [
