...
class Trainer(AbstractTrainer):
    ...
    def _build_optimizer(self, params):
        ...
        """
        if self.config[
                'reg_weight'] and self.weight_decay and self.weight_decay * self.config[
                    'reg_weight'] > 0:
            self.logger.warning(
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(
                    set_color('GPU RAM: ' + get_gpu_usage(self.device),
                              'yellow'))
        return total_loss
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses):
        ...
        train_loss_output = (set_color('epoch %d training', 'green') + ' [' +
                             set_color('time', 'blue') +
    ...
    def _add_train_loss_to_tensorboard(self,
                                       epoch_idx,
                                       losses,
                                       tag='Loss/Train'):
        if isinstance(losses, tuple):
    ...
    def fit(self,
        ...
        if 'dynamic' in self.config['train_neg_sample_args'].keys(
        ) and self.config['train_neg_sample_args']['dynamic'] != 'none':
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                self.tensorboard.add_scalar('Vaild_score', valid_score,
                                            epoch_idx)
    ...
    def _neg_sample_batch_eval(self, batched_data):
        ...
        if batch_size <= self.test_batch_size:
            origin_scores = self.model.predict(interaction.to(
                self.device))  # 调用predict()
        else:
        ...
        if self.config['eval_type'] == EvaluatorType.VALUE:
            ...
            scores = torch.full((batch_user_num, self.tot_item_num),
                                -np.inf,
                                device=self.device).type(torch.LongTensor)
            scores[row_idx.type(torch.LongTensor),
                   col_idx.type(torch.LongTensor)] = origin_scores.type(
                       torch.LongTensor)
            return interaction, scores, positive_u, positive_i
    ...
    def evaluate(self,
        ...
        if isinstance(eval_data, FullSortEvalDataLoader):
            ...
            if self.item_tensor is None:
                self.item_tensor = eval_data.dataset.get_item_feature().to(
                    self.device)
        else:
...
class KGTrainer(Trainer):
    ...
    def _train_epoch(self,
        ...
        elif epoch_idx % (self.train_rec_step +
                          self.train_kg_step) < self.train_rec_step:
        ...
        if interaction_state in [KGDataLoaderState.RSKG, KGDataLoaderState.RS]:
            return super()._train_epoch(train_data,
                                        epoch_idx,
                                        show_progress=show_progress)
...
class KGATTrainer(Trainer):
    ...
    def _train_epoch(self,
        ...
        rs_total_loss = super()._train_epoch(train_data,
                                             epoch_idx,
                                             show_progress=show_progress)
...
class DecisionTreeTrainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.checkpoint_dir = config['checkpoint_dir']
        ensure_dir(self.checkpoint_dir)
        temp_file = '{}-{}-temp.pth'.format(self.config['model'],
                                            get_local_time())
        self.temp_file = os.path.join(self.checkpoint_dir, temp_file)

        temp_best_file = '{}-{}-temp-best.pth'.format(self.config['model'],
                                                      get_local_time())
...
class xgboostTrainer(DecisionTreeTrainer):
    ...
    def _interaction_to_lib_datatype(self, dataloader):
        ...
        return self.xgb.DMatrix(data=data,
                                label=label,
                                silent=self.silent,
...
class lightgbmTrainer(DecisionTreeTrainer):
    ...
    def evaluate(self,
        ...
        self.deval_data, self.deval_label = self._interaction_to_sparse(
            eval_data)
...
class RecVAETrainer(Trainer):
    ...
    def _train_epoch(self,
        ...
        for epoch in range(self.n_dec_epochs):
            loss += super()._train_epoch(train_data,
                                         epoch_idx,
...
class FairFedTrainer(Trainer):
    r"""
    For step 1
    """
    def __init__(self, config, model):
        super(FairFedTrainer, self).__init__(config, model)

    def _full_sort_batch_eval_basic(self, batched_data):
        interaction, history_index, positive_u, positive_i = batched_data
        try:
            # Note: interaction without item ids
            origin_scores = self.model.full_sort_predict(
            ...
            new_inter.update(self.item_tensor.repeat(inter_len))
            if batch_size <= self.test_batch_size:
                origin_scores = self.model.predict(new_inter)
            else:
                origin_scores = self._spilt_predict(new_inter, batch_size)

        scores = origin_scores.view(-1, self.tot_item_num)
        scores[:, 0] = -np.inf
        if history_index is not None:
            scores[history_index] = -np.inf
        return interaction, origin_scores, scores, positive_u, positive_i

    def _neg_sample_batch_eval_basic(self, batched_data):
        interaction, row_idx, positive_u, positive_i = batched_data
        batch_size = interaction.length
        if batch_size <= self.test_batch_size:
            origin_scores = self.model.predict(interaction.to(
        ...
        if self.config['eval_type'] == EvaluatorType.VALUE:
            return interaction, origin_scores, origin_scores, positive_u, positive_i
        elif self.config['eval_type'] == EvaluatorType.RANKING:
            col_idx = interaction[self.config['ITEM_ID_FIELD']]
            batch_user_num = positive_u[-1] + 1
            scores = torch.full((batch_user_num, self.tot_item_num),
    ...
    def evaluate_basic(self,
        ...
        if isinstance(eval_data, FullSortEvalDataLoader):
            eval_func = self._full_sort_batch_eval_basic
            if self.item_tensor is None:
                self.item_tensor = eval_data.dataset.get_item_feature().to(
                    self.device)
        else:
            eval_func = self._neg_sample_batch_eval_basic  # √
        if self.config['eval_type'] == EvaluatorType.RANKING:
            self.tot_item_num = eval_data.dataset.item_num
        ...
        df_dict = {}
        for batch_idx, batched_data in enumerate(iter_data):
            interaction, origin_scores, scores, positive_u, positive_i = eval_func(
                batched_data)  # positive item的索引|positive item的ID
            #print('batched_data',batched_data)
            # 产生dataframe结构
            u = interaction['user_id'].cpu().numpy()
            i = interaction['item_id'].cpu().numpy()
            l = interaction['label'].cpu().numpy()
            s = origin_scores.cpu().numpy()
            s = np.around(s, 8)
            batch_arr = np.array([u, i, l, s]).T
            df_dict[batch_idx] = pd.DataFrame(
                batch_arr, columns=['uid', 'iid', 'label', 'score'])
            self.eval_collector.eval_batch_collect(scores, interaction,
                                                   positive_u, positive_i)

        df = pd.concat(list(df_dict.values()), ignore_index=True)
        df['uid'] = pd.DataFrame(df['uid'], dtype=np.int32)
        df['iid'] = pd.DataFrame(df['iid'], dtype=np.int32)
        df['score'] = round(df['score'], 8)

        self.eval_collector.model_collect(self.model)  # 未实现
        struct = self.eval_collector.get_data_struct()
        result = self.evaluator.evaluate(struct)
