...
class UGF(object):
    ...
    def _build_fairness_optimizer(group_df_list, k, metric, name='UGF'):
        ...
        try:
            ...
            for df in group_df_list:
                ...
                tmp_metric_list = [
                ]  # store the metric calculation for each user in current user group
                tmp_var_score_list = [
                ]  # store var * score for object function use
                for uid, group in df_group:
                    tmp_var_list = []  # store variables for sum(Qi) == k use
                    tmp_var_label_list = [
                    ]  # store var * label for recall calculation use
                    score_list = group['score'].tolist()
                    label_list = group['label'].tolist()
                    item_list = group['iid'].tolist()

                    for i in range(len(item_list)):
                        var_name = str(uid) + '_' + str(
                            item_list[i])  # variable name is "uid_iid"
                    ...
                    if metric == 'recall':
                        tmp_metric_list.append(
                            gp.quicksum(tmp_var_label_list) /
                            group['label'].sum())
                    elif metric == 'precision':
                        tmp_metric_list.append(
                            gp.quicksum(tmp_var_label_list) / k)
                    elif metric == 'f1':
                        f1 = 2 * gp.quicksum(tmp_var_label_list) / (
                            group['label'].sum() + k)
                        tmp_metric_list.append(f1)
                    else:
                        raise ValueError(
                            'Unknown metric for optimizer building.')

                metric_list.append(
                    gp.quicksum(tmp_metric_list) / len(tmp_metric_list))
    ...
    def _format_result(model, df):
        ...
        """
        # print(model.getVars())
        new_df = pd.DataFrame(model.getVars(),
                              columns=['uid_iid_q']).astype(str)
        new_df['uid'] = new_df['uid_iid_q'].map(
            lambda x: x.split(" ")[1].split("_")[0]).astype(int)
        new_df['iid'] = new_df['uid_iid_q'].map(
            lambda x: x.split(" ")[1].split("_")[1]).astype(int)
        new_df['q'] = new_df['uid_iid_q'].map(
            lambda x: x.split(" ")[3].split(")")[0]).astype(float)
    ...
    def _print_metrics(self, df, metrics, message='metric scores'):
        ...
        for i in range(len(metrics)):
            r_string = r_string + metrics[i] + "=" + '{:.4f}'.format(
                results[i]) + " "
    ...
    def train(self):
        ...
        all_df = self.data_loader.rank_df.copy(
            deep=True)  # the dataframe with entire test data
        ...
        # group 1 (beneficiaries of information difference)
        # group 2 (victims of information difference)

        # Print original evaluation results
        print('step1')
        self.logger.info(
            'Model:{} | Dataset:{} |  Epsilon={} | K={} | GRU_metric={}'.
            format(self.model_name, self.dataset_name, self.epsilon, self.k,
                   self.fairness_metric))
        result = self._print_metrics(
            all_df, self.eval_metric_list,
            'Before optimization overall scores           ')
        result1 = self._print_metrics(
            group_df_list[0], self.eval_metric_list,
            'Before optimization group 1 (active) scores  ')
        result2 = self._print_metrics(
            group_df_list[1], self.eval_metric_list,
            'Before optimization group 2 (inactive) scores')
        self.logger.info(
            "Before optimization ------------------ UFG-f1:{:.4f}".format(
                np.abs(result1[1] - result2[1])))
        print('\n')
        ...
        all_df['q'] = self._format_result(m, all_df)

        group_df_list[0].drop(columns=['q'], inplace=True)
        group_df_list[0] = pd.merge(group_df_list[0],
                                    all_df,
                                    on=['uid', 'iid', 'score', 'label'],
                                    how='left')
        group_df_list[1].drop(columns=['q'], inplace=True)
        group_df_list[1] = pd.merge(group_df_list[1],
                                    all_df,
                                    on=['uid', 'iid', 'score', 'label'],
                                    how='left')
        ...
        self.logger.info(
            "After optimization ------------------- UFG-f1:{:.4f}".format(
                np.abs(result1[1] - result2[1])))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--e', type=float, default=0.00,
                        help='epsilon')  # 0~0.1
    ...
    dataset_folder = 'C:/vscode_c/vscode_cc/code/data/'  # dataset directory
    data_path = os.path.join(dataset_folder, args.dataset)
    file_name = '_rank.csv'
    rank_file = args.model + file_name  # original input ranking csv file name
