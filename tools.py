...
def evaluation_methods(df, metrics):
    ...
    for metric in metrics:
        ...
        tmp_df = data_df.sort_values(by='q*s',
                                     ascending=False,
                                     ignore_index=True)
        ...
        if metric.startswith('ndcg@'):
            ...
            for uid, group in df_group:
                ndcgs.append(
                    ndcg_at_k(group['label'].tolist()[:k], k=k, method=1))
            ...
            for uid, group in df_group:
                if len(group['label'].tolist()) < k:
                    ...
                    print(uid)
                precisions.append(
                    precision_at_k(group['label'].tolist()[:k], k=k))
            ...
            for uid, group in df_group:
                if np.sum(group['label']) == 0:
                    continue
                recalls.append(1.0 * np.sum(group['label'][:k]) /
                               np.sum(group['label']))
            ...
            for uid, group in df_group:
                if np.sum(group['label']) == 0:
                    continue
                f1.append(2 * np.sum(group['label'][:k]) /
                          (np.sum(group['label']) + k))
